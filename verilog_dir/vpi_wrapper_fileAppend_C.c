 ...
 *     #include "vpi_wrapper_fileAppend_C.h"
 *     void (*vlog_startup_routines[])() = { fileAppend_C_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_fileAppend_C call=fileAppend_C_calltf size=1 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_fileAppend_C vpiSysFuncSized 1 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
/* sft: $imported_fileAppend_C vpiSysFuncSized 1 unsigned */

/* tab: $imported_fileAppend_C call=fileAppend_C_calltf size=1 acc=rw:%TASK */

PLI_INT32 fileAppend_C_sizetf(PLI_BYTE8 *user_data)
{
  return 1u;
}
...
void fileAppend_C_vpi_register()
{
  ...
  tf_data.tfname = "$imported_fileAppend_C";
  tf_data.calltf = fileAppend_C_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = fileAppend_C_sizetf;
  tf_data.user_data = "$imported_fileAppend_C";
